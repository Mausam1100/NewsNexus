...
export default class News extends Component {
  ...
  fetchArticles = async(page) => {
    let url = `https://newsapi.org/v2/everything?q=apple&from=2024-07-22&to=2024-07-22&sortBy=popularity&apiKey=6dcbbd9745d3465f9bc371bfb3b0a4a5&pageSize=${this.props.pageSize}&page=${page}`;
  ...
  render() {
    return (
      <div>
        <div className='w-[65%] max-w-[950px] mx-auto py-10'>
          ...
          <div className='w-full flex gap-9 flex-wrap items-stretch py-10'>
            {this.state.article.map((element) => {
              return <div key={element.url} className='w-[calc(34.5%-36px)]'>
                <NewsItems title={element.title} description={element.description.slice(0, 100)} imgUrl={element.urlToImage} newsUrl={element.url}/>
              </div>
            })}
          </div>
